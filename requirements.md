# Requirements
## 1. General System Requirements
- All displays throughout the game should follow a 4:3 aspect ratio with 1024 × 768 px.
- All displays have the same font/colour
- Background music and sound effects

## 2. Menu Requirements
- The Menu screen must display the game’s name "Sonder". 
- Menu display shows a graphic of a garden
- Menu display shows a welcoming written message, randomly selected 
- The Menu screen must display 3 buttons of "Start", "Garden", and "Entries".
- Clicking "Start" must direct to the start screen.
- Clicking "Garden" must direct to the garden screen.
- Clicking "Entries" must direct to the entries screen.

## 3. Start Requirements



- Task 1: 
- Task 2: 
- Task 3: 
  
## 4. Garden Requirements
- Always display the background of a garden image
- The garden take a 3*5 grid for planting flowers (maybe invisible?)
- Each cell of the grid holds one and only one flower
- New flowers can only added to empty cell
- After finishing task of the 15th day, a special flower will be planted on the last cell
- Flower type and position must be saved after user planted each flower

## 5. Entries Requirements


