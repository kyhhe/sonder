...
- Clicking "Start" must direct to the start screen, where the user may begin their daily tasks. 
...
- If the tasks have not been completed, the start screen directs the user to the next screen to complete the following tasks
   ...
   - Task 2: “What is one thing you’re grateful for?” Provides a textbox for the user to input their answer. Upon hitting ENTER, the user proceeds to the next task.
   - Task 3: “What is one thing you learned today? What is one goal you have for tomorrow?” →  Provides a textbox for the user to input their answer. Upon hitting ENTER, the user proceeds to the next task.
   - Task 4: “Choose a flower:” → user chooses one flower out of three options displayed on screen (randomly generated from a set of 14 total flowers, but does not include duplicate flowers that the user has selected from the previous day.
...
- The garden displays a 3*5 grid for planting flowers (maybe invisible?)
...
- After finishing the task on the 15th day, a special flower with unique visuals will be planted in the last cell
